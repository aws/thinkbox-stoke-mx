# Contributing to StokeMX

Thanks for your interest in contributing to StokeMX! ❤️
...
You will need Python 3.10 or later and the C++ compiler for your platform installed before you are able to build StokeMX. The compilers used for each platform are as follows:
...
You will need to install Intel MKL on your system in order to build Stoke as it is not available through Conan. You only need to build the SDK from thinkbox-krakatoa-mx and not the plugin itself.

## Building

From the project root directory run the following commands to install the dependencies and build StokeMX:
...
If you are using Windows, once run, you can open `build/StokeMX.sln` in Visual Studio to use Visual Studio for development and debugging.

### Publishing to Local Conan Cache

If you need to publish build artifacts to your local Conan cache manually, after completing the [building](#building) steps, you can run the following commands to package StokeMX and publish it to your local Conan cache:
